      ...
      filteredBooks.forEach((book) => {
        ...
        bookElement.innerHTML = `
                    ...
                    <div class="icons">
                        ...
                        ${
                          book.isComplete
                            ? `<img class='restore-icon' src="assets/restore icon.png" alt="Selesai" class="complete-icon">`
                            : `<img class='complete-icon' src="assets/checklist icon.svg" alt="Selesai" class="complete-icon">`
                        }
                    </div>
                `;
...
const deleteBook = (book, tab) => {
  ...
  const updatedBooks = storedBooks.filter(
    (storedBook) => storedBook.idBook !== bookId
  );
...
const restoreBook = (book, tab) => {
  const bookId = book.idBook;
  const storedBook = getStoredBooks() || [];
  const filteredBooks = storedBook.filter(
    (bookItem) => bookItem.idBook !== bookId
  );
  const updateItem = {
    ...book,
    isComplete: false,
...
const countBooks = () => {
  const storedBook = getStoredBooks() || [];
  const unreadTotal = storedBook.filter(
    (bookItem) => !bookItem.isComplete
  ).length;
